...
/**
 * Lex — proactive bubble + dock (DOC-52 §0.5, RF-VAN-005). Shared across all four
 * staff panels (admin · sales · legal · finance).
 *
 * V2.0 Lex is DETERMINISTIC: greetings and bubbles are computed by rules over the
 * real data already loaded (no LLM, P-52-07). The parent view passes the composed
 * text + actions; this component only renders + manages dismissal.
 *
 * - LexBubble: glass bubble with orb, rich text (<b> in accent), action row,
 *   close. Dismissed → does not reappear this session (module-level Set). The
 *   global "Lex bubbles" toggle (config) hides ALL bubbles via `enabled`.
 * - LexDock: 66px FAB orb (badge "1" when an unseen insight exists) + 360px glass
 *   panel with greeting message + quick-question pills.
 *
 * Self-contained: icons render via the `material-symbols-rounded` font already
 * loaded by the panel shell, and the `lexbub` / `lex-dock` classes live in the
 * global staff stylesheet — so no cross-feature imports are needed.
 */
...
/** Minimal Material Symbols glyph (font loaded once by the panel shell). */
function LexIcon({ name, size = 16 }: { name: string; size?: number }) {
  return (
    <span
...
function LexOrb({ size }: { size: number }) {
  return (
    <span className="lexbub-orb" style={{ width: size, height: size }} aria-hidden="true">
      <picture>
...
// Session-scoped dismissed set (per view key). Module-level so it survives route
// re-renders within the SPA session.
          ...
          <div className="lexbub-acts">
            {actions.map((a) => (
              <button
                ...
                onClick={a.onClick}
              >
                {a.icon && <LexIcon name={a.icon} size={15} />}
                {a.label}
              </button>
            ))}
          </div>
      ...
      {open && (
        <div className="lex-panel" role="dialog" aria-label="Lex, asistente">
          <div className="lex-panel-head">
            ...
            >
              <LexIcon name="close" size={18} />
            </button>
          </div>
          <div className="lex-msgs">
            <div className="lex-msg" dangerouslySetInnerHTML={{ __html: greetingHtml }} />
            {answers.map((m, i) => (
              <React.Fragment key={i}>
                <div className="lex-msg me">{m.q}</div>
                <div className="lex-msg" dangerouslySetInnerHTML={{ __html: m.a }} />
              </React.Fragment>
            ))}
          </div>
          <div className="lex-quick">
            {quickQuestions.map((q) => (
              <button
                ...
                type="button"
                className="lex-q"
                onClick={() => setAnswers((prev) => [...prev, { q: q.label, a: q.answer }])}
              >
                {q.label}
              </button>
            ))}
