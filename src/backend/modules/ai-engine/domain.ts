...
export const DEFAULT_GENERATION_RULES = [
  ...
  "R2. NEVER promote the client's characterization (e.g. \"they took my money\" does not become \"extortion\"; \"they hit me\" does not become \"torture\") unless a section is explicitly arguing that characterization as legal argument, flagged as argument and not fact. Likewise, NEVER reclassify a documented event as a legal status or figure the record does not literally state (e.g. an entry without inspection / \"EWI\" is not a \"deportation\" or \"removal\"; an I-94 admission class such as \"DT\"/deferred inspection is not \"humanitarian parole\" unless the record says so). Use the record's exact terms; when a legal status is not explicit in the record, describe the event neutrally or use the R6 placeholder rather than inferring it.",
