 ...
 * @supabase/ssr cookie value (base64url) to valentina-cookie.json (gitignored).
 */
const fs = require("fs");
...
const EMAIL = "valentina.apelacion.demo@example.com";
...
(async () => {
  ...
  fs.writeFileSync(path.join(__dirname, "valentina-cookie.json"), JSON.stringify({ name: cookieName, value: val }));
