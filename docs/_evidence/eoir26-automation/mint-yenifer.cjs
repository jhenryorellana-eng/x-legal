 ...
 * @supabase/ssr cookie value (base64url) to yenifer-cookie.json (gitignored).
 */
const fs = require("fs");
...
const EMAIL = "yenifer.reforzar.demo@example.com";
...
(async () => {
  ...
  fs.writeFileSync(path.join(__dirname, "yenifer-cookie.json"), JSON.stringify({ name: cookieName, value: val }));
