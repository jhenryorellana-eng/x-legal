/* Fase 2.1 — Seed the memo input documents for the asilo case: Declaración jurada +
 * Evidencias sustentatorias, with deterministic completed document_extractions.
 * Verbatim narrative/PDF logic from docs/_evidence/f-karelis/mint-docs.cjs; CASE_ID +
 * UPLOADED_BY read from asilo-ids.json. Run: node docs/_evidence/prod-verify/40-seed-docs-asilo.cjs */
const fs = require("fs");
...
const PHASE_ID = "10218501-fde6-488a-a11a-8b9ed4c41fc6"; // asilo principal
...
const AFFIDAVIT = {
  ...
  payload: {
    declarant_name: "Karelis Andreína Pérez", declaration_date: "2025-09-20", nationality: "Venezuelan",
...
const EVIDENCES = [
  { letter: "A", filename: "evidencia_A_amenaza", display: "Evidencia A - Mensaje de amenaza", docType: "threat letter", date: "2024-02-18", source: "Anonymous text message", title: "EVIDENCIA A - TRANSCRIPCION DE MENSAJE DE AMENAZA",
    paras: ["Fecha: 18 de febrero de 2024. Medio: mensaje de texto recibido en el telefono de Karelis Andreina Perez desde un numero desconocido.", "Transcripcion literal: Sabemos quien eres y donde vive tu familia. Deja de publicar mentiras contra la revolucion o atente a las consecuencias. Es tu ultima advertencia.", "Nota: este mensaje fue reportado al Sindicato Nacional de Trabajadores de la Prensa (SNTP) el mismo dia de su recepcion."] },
  { letter: "B", filename: "evidencia_B_denuncia_cicpc", display: "Evidencia B - Denuncia CICPC", docType: "police report", date: "2024-09-13", source: "CICPC, Sub-delegacion Caracas", title: "EVIDENCIA B - CONSTANCIA DE DENUNCIA (CICPC)",
    paras: ["Cuerpo de Investigaciones Cientificas, Penales y Criminalisticas (CICPC), Sub-delegacion Caracas.", "Expediente No. K-24-0154-00987. Fecha de la denuncia: 13 de septiembre de 2024.", "Denunciante: Karelis Andreina Perez, titular de la cedula de identidad venezolana.", "Hechos denunciados: la denunciante manifiesta que el 12 de septiembre de 2024, siendo aproximadamente las 5:30 horas, sujetos armados no identificados irrumpieron en su domicilio, sustrajeron equipos de computo y documentos de trabajo, y profirieron amenazas contra su vida y la de su familia.", "(Documento ficticio elaborado unicamente para fines de demostracion del sistema.)"] },
  { letter: "C", filename: "evidencia_C_informe_medico", display: "Evidencia C - Informe medico", docType: "medical report", date: "2024-09-14", source: "Centro medico (ficticio)", title: "EVIDENCIA C - INFORME MEDICO",
    paras: ["Centro medico (ficticio). Fecha de evaluacion: 14 de septiembre de 2024.", "Paciente: Karelis Andreina Perez, 36 anos de edad.", "Motivo de consulta: contusiones en el brazo derecho y cuadro de ansiedad aguda posterior a un evento de violencia ocurrido en su domicilio.", "Hallazgos: hematomas compatibles con sujecion forzada del antebrazo; sintomas compatibles con estres post-traumatico. Se indica reposo relativo y apoyo psicologico.", "(Informe ficticio elaborado unicamente para fines de demostracion del sistema.)"] },
  { letter: "D", filename: "evidencia_D_nota_prensa", display: "Evidencia D - Nota de prensa (condiciones del pais)", docType: "press article", date: "2025-01-15", source: "Reportaje de prensa (ficticio)", title: "EVIDENCIA D - NOTA DE PRENSA / CONDICIONES DEL PAIS",
    paras: ["Titular: Organizaciones denuncian aumento de la persecucion contra periodistas en Venezuela. Fecha: enero de 2025.", "Resumen: organizaciones de derechos humanos y de libertad de prensa documentan un patron creciente de detenciones arbitrarias, allanamientos y amenazas contra periodistas de investigacion en Venezuela, atribuidos a organismos de seguridad del Estado como el SEBIN. El informe describe censura, cierre de medios y el uso de cargos de instigacion al odio para criminalizar el trabajo periodistico.", "(Articulo ilustrativo para fines de demostracion; durante la generacion, la IA debe corroborar el patron con fuentes publicas reales y verificables.)"] },
  { letter: "E", filename: "evidencia_E_carta_testigo", display: "Evidencia E - Carta de testigo", docType: "witness letter", date: "2025-07-10", source: "Colega del SNTP (ficticio)", title: "EVIDENCIA E - CARTA DE TESTIGO",
    paras: ["Fecha: 10 de julio de 2025.", "Yo, colega y miembro del Sindicato Nacional de Trabajadores de la Prensa (SNTP), hago constar que conozco a Karelis Andreina Perez desde hace mas de diez anos como colega en el diario El Nacional.", "Doy fe de que fui testigo de las amenazas que ella comenzo a recibir a partir de febrero de 2024 y de que, tras el allanamiento de su vivienda en septiembre de 2024, la vi con lesiones visibles y en evidente estado de temor por su vida. Sus reportajes sobre corrupcion son ampliamente conocidos en el gremio periodistico.", "(Carta ficticia elaborada unicamente para fines de demostracion del sistema.)"] },
];

const PAGE_W = 612, PAGE_H = 792, MARGIN = 56, SIZE = 11, LH = 15.5;
function wrap(text, font, size, maxWidth) { const words = text.split(/\s+/); const lines = []; let line = ""; for (const w of words) { const trial = line ? line + " " + w : w; if (font.widthOfTextAtSize(trial, size) > maxWidth && line) { lines.push(line); line = w; } else line = trial; } if (line) lines.push(line); return lines; }
async function buildPdf(title, paras) {
  ...
  const newline = (h) => { y -= h; if (y < MARGIN + LH) { page = pdf.addPage([PAGE_W, PAGE_H]); y = PAGE_H - MARGIN; } };
  for (const tl of wrap(title, bold, 14, maxW)) { page.drawText(tl, { x: MARGIN, y, size: 14, font: bold, color: rgb(0.1, 0.12, 0.22) }); newline(20); }
  newline(8);
  for (const p of paras) { for (const ln of wrap(p, font, SIZE, maxW)) { page.drawText(ln, { x: MARGIN, y, size: SIZE, font, color: rgb(0, 0, 0) }); newline(LH); } newline(7); }
  return Buffer.from(await pdf.save());
}
const rawTextOf = (title, paras) => title + "\n\n" + paras.join("\n\n");

(async () => {
  const sb = createClient(URL, SERVICE_ROLE, { auth: { persistSession: false } });
  const { data: prior } = await sb.from("case_documents").select("id, storage_path").eq("case_id", CASE_ID).in("required_document_type_id", [TYPE_DECLARACION, TYPE_EVIDENCIAS]);
  ...
  const items = [];
  items.push({ typeId: TYPE_DECLARACION, filename: "declaracion_jurada_karelis", display: "Declaración jurada de Karelis", title: AFFIDAVIT.title, paras: AFFIDAVIT.paras, payload: AFFIDAVIT.payload });
  ...
  const packetParas = ["Este paquete consolida las evidencias A a E presentadas en apoyo de la solicitud de asilo de Karelis Andreina Perez. Cada seccion corresponde a una evidencia individual."];
  for (const e of EVIDENCES) { packetParas.push("== " + e.title + " =="); for (const p of e.paras) packetParas.push(p); }
  items.push({ typeId: TYPE_EVIDENCIAS, filename: "evidencias_consolidado_A_E", display: "Evidencias sustentatorias - Paquete consolidado A-E", title: "PAQUETE CONSOLIDADO DE EVIDENCIAS A-E", paras: packetParas, payload: { document_type: "consolidated packet", document_date: "2025-09-20", author_or_source: "Compilado por el equipo legal", summary: "Paquete consolidado de las evidencias A-E (amenaza, denuncia CICPC, informe medico, nota de prensa, carta de testigo)." } });
  ...
  console.log("\nDONE " + items.length + " documents created (consolidated packet last = newest).");
