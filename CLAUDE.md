...
- **Verificación en vivo con navegador**: MCP de Playwright (`mcp__plugin_playwright_playwright__browser_*`). Login staff real: `vanessa@usalatinoprime.com` / `changeme-vanessa!` (sales) o Henry admin. **`/admin` es role-aware**: no-admins redirigen a su panel (`/ventas/mi-dia`, etc.).
- **Sesión de cliente para E2E/verificación** (el login es email-OTP, sin SMTP): acuñar con `signInWithPassword` (María `maria.gonzalez.demo@example.com` / `demo-maria!`) y construir la cookie SSR `base64-<base64(JSON sesión)>` para inyectarla en el navegador (ver `docs/_evidence/f3-verify/mint-maria.cjs`). storageState E2E en `e2e/.auth/` (gitignored).
