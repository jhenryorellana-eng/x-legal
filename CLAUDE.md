...
- **Verificación en vivo con navegador**: MCP de Playwright (`mcp__plugin_playwright_playwright__browser_*`). Login staff real en `/login`: Henry admin `henry@usalatinoprime.com` / `changeme-henry!`, `vanessa@usalatinoprime.com` / `xlegal-vanessa2026!` (sales, contraseña rotada 2026-07-14 — la de Henry sigue con el patrón viejo, NO se rotó globalmente; si un login falla, verifica el hash vigente con `pgcrypto`: `select encrypted_password = crypt('<candidata>', encrypted_password) from auth.users where email='<correo>'`). **`/admin` es role-aware**: no-admins redirigen a su panel (`/ventas/mi-dia`, etc.).
